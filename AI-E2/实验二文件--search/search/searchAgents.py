...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):     
        ...
        if (x, y) == self.corners[0]:
            Cornor = (True, False, False, False)#起始位置是不是在角落上
        elif (x, y) == self.corners[1]:
            Cornor = (False, True, False, False)
        elif (x, y) == self.corners[2]:
            Cornor = (False, False, True, False)
        elif (x, y) == self.corners[3]:
            Cornor = (False, False, False, True)
        else:
            Cornor = (False, False, False, False)
        return (self.startingPosition, Cornor)
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        c = state[1]
        Is = c[0] and c[1] and c[2] and c[3] #四个角落全走过了
        return Is
        util.raiseNotDefined()
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            Nxt = self.getNextState(state, action)
            children.append((Nxt, action, self.getActionCost(state, action, Nxt)))
    ...
    def getNextState(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        nowState = state[1]
        nexState = (nextx, nexty)
        if nexState == (1, 1):#如果走到了角落
            Cornors = (True, nowState[1], nowState[2], nowState[3])
        elif nexState == (1, self.top):
            Cornors = (nowState[0], True, nowState[2], nowState[3])
        elif nexState == (self.right, 1):
            Cornors = (nowState[0], nowState[1], True, nowState[3])
        elif nexState == (self.right, self.top):
            Cornors = (nowState[0], nowState[1], nowState[2], True)
        else:
            Cornors = nowState#没走到角落
        #util.raiseNotDefined()
        # you will need to replace the None part of the following tuple.
        return ((nextx, nexty), Cornors)
...
def Dis(pos, tar):
    return abs(pos[0] - tar[0]) + abs(pos[1] - tar[1])
...
def cornersHeuristic(state, problem):
    ...
    candi = util.PriorityQueue()
    pos, cr_state = state
    Sum = 0
    cnt = 0
    for i in range(4):  #将未搜索到的角落压入优先队列，优先搜索最近的
        if cr_state[i] == False:
            candi.push(corners[i], Dis(pos, corners[i]))
            cnt += 1
    now = pos
    for i in range(cnt): #到达角落后在优先队列中删除这个点，再比较当前位置与剩余未到角落与之的距离，作为启发函数的值
        node = candi.pop()
        Sum += Dis(node, now)
        now = node
        lst = []
        ...
        for j in range(cnt - i - 1):
            candi.pop()
        for j in range(cnt - i - 1):
            candi.push(lst[j], Dis(lst[j], now))
    return Sum
...
def foodHeuristic(state, problem):
    ...
    for food_pos1 in foodGrid.asList():
        for food_pos2 in foodGrid.asList():
            Sum = max(Sum, Dis(food_pos1, food_pos2));  #计算最远点对距离
    return Sum
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        return search.bfs(problem)  #直接使用BFS即可
        util.raiseNotDefined()
